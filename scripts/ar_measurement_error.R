...
stan_data <- list(
  ...
  T   = T_obs,
  F   = ceiling(T_obs * 0.2),
  y   = as.vector(arima.sim(list(ar=true_beta, sd = true_sigma, m = 0), T_obs)) + rnorm(T_obs, 0, true_tau),
  tau = true_tau 
)
...
# Fit the data ----
ar_measurement_error_stan_model <- rstan::stan_model(here::here("stan", "ar_measurement_error.stan"))
ar_stan_fit <- rstan::sampling(ar_measurement_error_stan_model, stan_data, cores = 4)
